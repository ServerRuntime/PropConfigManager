...
public class LogController {
    ...
    public SseEmitter streamLogs(
        ...
        if (opt.isEmpty()) { completeWithError(emitter, "Makine bulunamadı: " + machineId); return emitter; }
        ...
        final String user = creds[0], pass = creds[1];
        ...
        executor.submit(() -> {
            try {
                ...
                try { emitter.send(SseEmitter.event().name("sse-error").data(e.getMessage())); } catch (Exception ignored) {}
                emitter.complete();
            }
        });
    ...
    private String[] resolveCreds(Machine m, String u, String p) {
        if (m.isHasCredentials()) return new String[]{m.getUsername(), m.getPassword()};
        if (u != null && !u.isBlank() && p != null && !p.isBlank()) return new String[]{u, p};
        return null;
    }

    private void completeWithError(SseEmitter emitter, String msg) {
        try { emitter.send(SseEmitter.event().name("sse-error").data(msg)); } catch (Exception ignored) {}
        emitter.complete();
    }
}
